...
export async function GET(
  ...
  try {
    ...
    const rateCheck = await checkRateLimit(`doc_json_${ip}`, { limit: 60, windowMs: 60 * 1000 });
    ...
    const cleanNum = numero.toUpperCase().trim();
    ...
    return NextResponse.json(
      { success: false, message: "Erreur lors du chargement des données du document." },
      { status: 500 }
    );
  }
}
